...
let rec ty_of_array (c : ctxt) (l:Range.t Ast.lhs): ty =
  begin match l with
    |Ast.Var(_, id) ->
    ...
    |Ast.Index(lhs, exp) -> (ty_of_array c lhs)
  end
  ...
  match l with
	...
	let (ans_id, ans_op) = gen_local_op I32 "len" in 
      ...
      let arr_ty =
      	...
      	|Ptr p -> p
      	|_->failwith "dereferencing but not a pointer"
      end in
	...
	begin match (fst array_op) with
	  |Ptr (Ptr p)->
	    begin match p with
	      |Struct (h1::h2::t)->begin match h2 with
		  | Array (i,x)->x
		  |_->failwith "Not the array we want"
		    end
	      |_->failwith "Not the struct we want"
	    end	    
